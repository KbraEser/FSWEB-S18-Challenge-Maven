...
public class CardRepositoryImpl implements CardRepository{
    ...
    public List<Card> findByColor(String color) {
        TypedQuery<Card> query = entityManager.createQuery("SELECT c FROM color c WHERE c.color :color", Card.class);
        query.setParameter("color", color);

        List<Card> result = query.getResultList();

        if(result == null || result.isEmpty()){
            throw new CardException("Card with color"+ color + "not found", HttpStatus.NOT_FOUND);
        }
    ...
    public Card remove(Long id) {
        Card existingCard=entityManager.find(Card.class,id);
